...
class CodeTokensASTEncoder(Encoder):
    ...
    def finalise_metadata(cls, encoder_label: str, hyperparameters: Dict[str, Any],
        ...
        code_encoder_metadata_list = list(
            map(lambda raw_metadata: raw_metadata[cls.CODE_ENCODER_LABEL], raw_metadata_list))
        ast_encoder_metadata_list = list(
            map(lambda raw_metadata: raw_metadata[cls.AST_ENCODER_LABEL], raw_metadata_list))
        return {
